...
**Problem.** People learning Linux, trying a tool, or running a quick experiment want a shell *now*, without installing anything and without being able to damage anything. Hosted products solve this (GitHub Codespaces, Killercoda, Replit). The interesting engineering is in the host: many users share one machine, every container is a potential leak, and the moment a spawn is slow or a neighbour is noisy every user feels it. Those are exactly the problems observability is for, which is why I picked this over a CRUD app.
...
- Click **New sandbox** → `POST /sessions` (anonymous token) → `POST /sessions/{id}/sandbox` → a slot in the pool of 10 (queue up to 60 s if full; the page shows the queue position and Destroy cancels) → a *warm* pre-created container is claimed (~0 ms) or a *cold* one is created (~0.4–1.5 s) → `WS /ws/{id}` attaches `bash -l`. Reloading the page reattaches a new shell to the same sandbox.
...
- **A second bug found by using it.** Clicking *New sandbox* repeatedly created a new anonymous session each time and left the previous sandbox running until the 15-minute idle reap, so one person could fill the whole pool and then queue behind their own boxes. The Business dashboard showed it plainly (active 10/10, queue 1, one browser). Fix: the page keeps its session in `sessionStorage` and reattaches on reload, destroys the previous sandbox before requesting a new one, and `DELETE /sessions/{id}` cancels a request that is still queued (`QueueCancelled` in `api/pool.py`, `queue_position` in `GET /sessions/{id}`). The same family had two more members that a review found later: a page reloaded *while queued* forgot its request, which then took a slot nobody owned (the page now DELETEs it on unload/resume), and a DELETE landing while `docker create` was in flight left the new container and the slot behind (`request_sandbox` now checks for that; `test_delete_while_spawning_returns_slot_and_container`).
- **Two things the pipeline caught that I would not have noticed otherwise.** uvicorn logged every WebSocket handshake URL, token included, on its `uvicorn.error` logger (Kibana `message : "[accepted]"`; see C.1) — and Node Exporter's network panel was measuring the exporter container's own veth until it got `network_mode: host` (B.6). Both were only visible because the data was in Kibana and Grafana.
