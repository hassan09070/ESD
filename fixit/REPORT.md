...
The LLM is pluggable: `OllamaLLM` (a local model such as `qwen2.5-coder:7b` through Ollama's `/api/chat` tool calling, running on the host with Metal acceleration and reached from the container at `host.docker.internal:11434`), `AnthropicLLM` (Claude `claude-sonnet-4-6` via the Messages API) or `MockLLM`, a scripted, deterministic agent that fixes the sample repo in exactly 5 calls with 0.2 s of simulated latency each. All experiments use the mock so they are free and repeatable. A `FaultInjectingLLM` wrapper (env `FIXIT_FAULT=none|slow|flaky`) is the switch for Part E.

**What works.** The whole stack starts from `docker compose up -d` with no API key. With Ollama and `qwen2.5-coder:7b` (a 4.7 GB local model) the agent fixes the sample repo unaided in 8 iterations and ~50 s, including recovering from its own wrong first attempt (`a / b - 1`) after reading the failing assertion. The agent fixes `sample_repo` (calculator with a swapped-argument bug and an off-by-one bug; 2 of 5 tests failing) in 5 iterations, ~2 s, $0.0375 of (simulated) tokens. 27 unit/integration tests pass. Metrics reach Grafana, logs reach Kibana, both experiments run end-to-end from scripts.
